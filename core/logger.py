...
class VisualWriter():
    ...
    def save_images(self, results,itr = None):
        ...
        try:
            ...
            for i in range(len(names)):
                resulta = os.path.join(result_path,names[i])
                os.makedirs(resulta, exist_ok=True)
                nm = str(itr)+'.png'
                Image.fromarray(outputs[i]).save(os.path.join(resulta, nm))
        except:
